...
class ProductBloc extends BlocBase {
  ...
  Future<bool> saveProduct() async {
   ...
   try {
     if(product != null){
       _uploadImages(product.documentID);
       await product.reference.updateData(unsavedData);
     } else {
       DocumentReference ref = await Firestore.instance.collection("products").document(categoryId)
           .collection("itens").add(Map.from(unsavedData)..remove("images"));
       await _uploadImages(ref.documentID);
       await ref.updateData(unsavedData);
     }
     _createdController.add(true);
     _loadingController.add(true);
     return true;
